...
$(`#botonCarrito`).on(`click`, function () {
    ...
    if (localStorage.setItem("carrito", JSON.stringify(carrito))) {
        for (let x = 0; x <= localStorage.length; x++) {
            ...
            guardado = localStorage.getItem("carrito", JSON.stringify(carrito));
        }
        return
    }
    else {
...
//localStorage.setItem("carrito", guardado.stringify(carrito));
//LEER datos de localStorage:
let guardado = localStorage.getItem("carrito", JSON.stringify(carrito));
